...
def prepare_data(data, labels, class_to_filename_dict, frame_rate, size_window, step_for_window):
    ...
    for instance_idx in range(data.shape[0]):
        ...
        for windows_idx in range(num_windows - 1):
            ...
            new_labels[instance_idx, windows_idx] = temp_labels[
                                                    start_idx_label:start_idx_label + length_of_label_window, 1]
            new_labels_timesteps[instance_idx, windows_idx] = temp_labels[
        ...
        if start_idx_data + size_window >= data.shape[1]:
            ...
            new_labels[instance_idx, num_windows - 1] = temp_labels[
                                                        temp_labels.shape[0] - length_of_label_window:temp_labels.shape[
                                                            0], 1]
...
def concatenate_prediction(true_values, predicted_values, timesteps_labels, class_dict):
    ...
    for instance_idx in range(predicted_values.shape[0]):
        ...
        for timestep in timesteps:
            ...
            result_predicted_values.iloc[index_temp, 2] = np.mean(
                predicted_values[instance_idx, timesteps_labels[instance_idx] == timestep])
            index_temp += 1
