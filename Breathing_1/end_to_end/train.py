...
lengths = [64000, 96000, 128000, 160000, 192000, 224000, 256000, 512000]
batch_sizes = [60, 40, 30, 24, 19, 17, 15, 7]
best_results = []
for num_run in range(10):
    ...
    for i in range(epochs):
        ...
        if i % 1 == 0:
            ...
            concatenated_predicted_labels = new_concatenate_prediction(true_values=val_labels,
            ...
            print('iteration:', i, '     pearson_coef:', prc_coef, '  processing time for epoch:', end - start)
    ...
    best_results.append([num_run, best])
    pd.DataFrame(columns=['num_run', 'best_result'], data=best_results).to_csv(path_to_best_model + 'best_results.csv',
                                                                               index=False)
    del model
    K.clear_session()
    gc.collect()
